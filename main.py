...
def train(args):
    ...
    with open('./weights/log.csv', 'w') as f:
        best = 0
...
def test(args, model=None):
    if model is None:
        model = torch.load('./weights/resnet_152.pt', 'cuda')['model'].float()
    model.eval()
    model.to(device)
...
def main():
    ...
    parser.add_argument('--data-dir', type=str, default='../../Datasets/imagenet')
    parser.add_argument('--model-name', type=str, default='152')
